...
from src.schemas import TowAirplaneSchema, TowAirplaneSearchSchema, TowAirplaneCreateSchema, TowAirplaneUpdateSchema
...
@app.post(
    f"/{prefix}/search",
    tags=tags,
    response_model=List[TowAirplaneSchema],
    summary=f"Search {prefix}",
)
async def search(
        page: int = 1,
        page_size: Optional[int] = None,
        filters: Optional[TowAirplaneSearchSchema] = None,
...
@app.post(
    f"/{prefix}",
    tags=tags,
    response_model=TowAirplaneSchema,
    summary=f"Create {prefix}",
)
async def create(
        data: TowAirplaneCreateSchema, db: Session = Depends(get_db)
):
    """
    Create tow_airplane
    :param data: Data
...
async def get_by_id(id_: int, db: Session = Depends(get_db)):
    """
    Read tow_airplane by ID
    :param id_: TowAirplane ID
    :param db: Database session
    :return: TowAirplane
    """
    tow_airplane = await crud.get_by_id(db=db, id_=id_)
    if not tow_airplane:
