...
from src.schemas import MemberSchema, MemberSearchSchema, MemberCreateSchema, MemberUpdateSchema
...
async def create(
        data: MemberCreateSchema, db: Session = Depends(get_db)
):
    """
    Create member
    :param data: Data
...
async def get_by_id(id_: int, db: Session = Depends(get_db)):
    ...
    :return: Member
    """
    member = await crud.get_by_id(db=db, id_=id_)
    if not member:
